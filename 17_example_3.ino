...
void loop()
{
  ...
  if (dist_raw < _DIST_MIN) dist_filtered = _DIST_MIN;
  else if (dist_raw > _DIST_MAX) dist_filtered = _DIST_MAX;
  else dist_filtered = dist_raw;

    if (a_value > 0 && dist_raw >= _DIST_MIN && dist_raw <= _DIST_MAX)
  ...
  duty = (int)(_DUTY_MIN + ((_DUTY_MAX - _DUTY_MIN) * (dist_ema - _DIST_MIN)) / (_DIST_MAX - _DIST_MIN));
